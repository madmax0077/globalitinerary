 ...
 * Matches the shape of destination-guides.ts (Bali / Dubai / London style posts).
...
/**
 * Optional extra days drawn from our own Top 100 “why go” notes — not invented
...
function buildArticle(city: City, rank: number, why: string): Article {
...
export const top100CityGuides: Article[] = TOP_100_CITIES.flatMap((seed) => {
  ...
  if (!city) return [];
  return [buildArticle(city, seed.rank, seed.why)];
});
