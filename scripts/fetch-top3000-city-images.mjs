/**
 * Fetch Wikipedia/Commons images for newly added top-3000 tourist cities.
 * Writes images-map.json + gallery-map.json (generate-cities.mjs reads those).
 *
 * Run: $env:NODE_TLS_REJECT_UNAUTHORIZED=0; node scripts/fetch-top3000-city-images.mjs
 */
import fs from "node:fs";
import path from "node:path";
import { fileURLToPath } from "node:url";
import { isScenic } from "./lib/bad-image.mjs";
import { TOP3000_EXTRA } from "./top3000-tourism-cities.mjs";
...
console.log("top3000 extras needing real photos:", need.length);
...
for (const { entry, target: t, title } of need) {
...
checkpoint();
fs.writeFileSync(
  path.join(__dirname, "fetch-top3000-images-log.json"),
