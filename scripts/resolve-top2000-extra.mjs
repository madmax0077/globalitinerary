/**
 * Resolve Wikipedia coordinates for top2000 candidates → top2000-extra.txt
 * Run: $env:NODE_TLS_REJECT_UNAUTHORIZED=0; node scripts/resolve-top2000-extra.mjs
 */
...
const candidates = fs
  .readFileSync(path.join(__dirname, "top2000-candidates.filtered.txt"), "utf8")
  .split(/\r?\n/)
  .map((l) => l.trim())
...
const failed = [];
const header =
  "# Top tourism cities beyond uniqueTop1500. Format:\n# slug|name|countrySlug|CC|countryName|lat|lng|timezone|population|isCapital|wikiTitle?\n";

function writePartial() {
  fs.writeFileSync(path.join(__dirname, "top2000-extra.txt"), header + out.join("\n") + "\n");
}
...
writePartial();
fs.writeFileSync(
  path.join(__dirname, "top2000-resolve-log.json"),
