/**
 * Loop: fetch Wikipedia/Commons images for cities missing real photos,
 * validate scenic + place relevance, write into cities.generated.ts + images-map.
 *
 * Run: npx tsx scripts/fetch-and-apply-city-images.mjs
 * Env: FETCH_LIMIT=100 (optional), NODE_TLS_REJECT_UNAUTHORIZED=0 if needed
 */
...
const LIMIT = Number(process.env.FETCH_LIMIT || 0) || Infinity;
const SLEEP_MS = 350;
...
function placeRelevant(url, city) {
  ...
  // Accept if filename mentions city token or country (for capitals/skylines)
  if (tokens.some((t) => f.includes(t))) return true;
  if (country && country.length >= 4 && f.includes(country.split(" ")[0])) return true;
  // Scenic travel keywords — OK if page was the city article (trust wiki title match)
  if (/(skyline|city|view|panorama|street|harbour|harbor|beach|temple|mosque|church|castle|palace|square|plaza|market|river|lake|mountain|coast|bridge|tower|fort|old.?town)/i.test(file)) {
    return true;
  }
  // Reject celebrity/unrelated
  if (/(hilton|portrait|painting|map|stamp|coa|flag|logo|diagram)/i.test(file)) return false;
  // Soft accept: any scenic wiki from the correct page
  return isScenic(url);
}
...
async function wikiPageImages(title) {
  ...
  let finalTitle = page.title || title;
  return { hero, files, finalTitle };
}
...
async function findImages(city, wikiTitle) {
  ...
  if (!best.hero) {
    ...
    if (ok.length) {
      best = { hero: ok[0], gallery: ok.slice(0, 6), title: "commons:" + city.name };
    }
  }
  return best;
}
...
const need = data.filter((c) => {
  if (CURATED_KEEP_STOCK.has(c.slug)) return false;
  const has = c.realPhoto && isScenic(c.realPhoto) && placeRelevant(c.realPhoto, c);
  return !has;
});

console.log("cities needing photos", need.length, "limit", LIMIT === Infinity ? "all" : LIMIT);

let updated = 0;
let failed = 0;
const log = [];

for (const c of need.slice(0, LIMIT)) {
  const t = bySlug.get(c.slug);
  const wikiTitle = t?.wikiTitle || c.name;
  ...
  try {
    ...
    updated++;
    console.log("OK", found.hero.slice(0, 70));
  ...
  if (updated > 0 && updated % 25 === 0) {
    ...
    console.log("checkpoint saved", updated);
  }
}
...
fs.writeFileSync(path.join(__dirname, "gallery-map.json"), JSON.stringify(galleryMap, null, 2));
fs.writeFileSync(cityFile, before + JSON.stringify(data, null, 2) + after);
fs.writeFileSync(path.join(__dirname, "fetch-city-images-log.json"), JSON.stringify({ updated, failed, log }, null, 2));
