/**
 * Fetch Wikipedia/Commons images for top-1000 tourist cities still on stock.
 * Real Wikimedia photos only â€” scenic + place-relevance filtered.
 *
 * Run: $env:NODE_TLS_REJECT_UNAUTHORIZED=0; node scripts/fetch-top1000-city-images.mjs
 */
import fs from "node:fs";
import path from "node:path";
import { fileURLToPath } from "node:url";
import { isScenic } from "./lib/bad-image.mjs";
import { TOP2500_EXTRA } from "./top2500-tourism-cities.mjs";
...
console.log("top2500 extras needing real photos:", need.length);
...
fs.writeFileSync(cityFile, before + JSON.stringify(data, null, 2) + after);
fs.writeFileSync(
  path.join(__dirname, "fetch-top2500-images-log.json"),
