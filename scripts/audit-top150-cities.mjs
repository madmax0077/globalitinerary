/**
 * Audit top 150 tourism cities vs city-targets + curated cities.
...
// Also accept turkey <-> turkiye for matching
...
function find(entry) {
  ...
  for (const s of candidates) {
    ...
    if (curatedSlugs.has(s)) {
      // curated cities are trusted for known slugs
...
console.log("TOP150 unique:", list.length);
...
fs.writeFileSync(new URL("./top150-audit.json", import.meta.url), JSON.stringify(out, null, 2));
console.log("\nWrote scripts/top150-audit.json");
