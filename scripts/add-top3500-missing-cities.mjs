/**
 * Add missing top-3500 tourism cities to city-targets.json, then:
 *   node scripts/generate-cities.mjs
 *   $env:NODE_TLS_REJECT_UNAUTHORIZED=0; node scripts/fetch-wikivoyage-missing.mjs
 *   node scripts/generate-cities.mjs
 *   $env:NODE_TLS_REJECT_UNAUTHORIZED=0; node scripts/fetch-top3500-city-images.mjs
 *   node scripts/generate-cities.mjs
 *   node scripts/generate-city-sitemap-slugs.mjs
 */
import fs from "node:fs";
import { uniqueTop3500 } from "./top3500-tourism-cities.mjs";
...
const list = uniqueTop3500().map((c) => ({
  ...c,
  countrySlug: c.countrySlug === "czech-republic" ? "czechia" : c.countrySlug === "turkey" ? "turkiye" : c.countrySlug,
}));
