...
export function isScenic(u) {
  if (typeof u !== "string" || !u.length) return false;
  // Heroes/galleries must be raster photos — never audio/video/SVG/PDF.
  if (!/\.(jpe?g|png|webp)(?:\?|$)/i.test(u) && !/\/thumb\/.+\.(jpe?g|png|webp)\//i.test(u)) {
    return false;
  }
  return !BAD_IMAGE.test(u);
}
