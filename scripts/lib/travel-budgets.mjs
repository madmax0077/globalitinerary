/**
 * Realistic mid-range daily ground budgets (USD per person) for travel planning.
 * Sources: Numbeo-style cost of living + backpacker/mid-range travel guides (approx).
 * budgetPerDay display = mid-range low–high; tripCost tiers are more granular.
 */

/** @typedef {{ low: number, high: number, currency?: string, note?: string }} Band */
/** @typedef {{ budget: [number, number], mid: [number, number], luxury: [number, number], currency: string, note: string }} Tiers */

/** Country slug → mid-range daily USD [low, high] */
export const COUNTRY_MID_USD = {
  // Western / Nordics / Oceania (expensive)
  switzerland: [180, 350],
  norway: [160, 320],
  iceland: [170, 340],
  denmark: [140, 280],
  ...
  // Africa / Middle East value / mixed
  morocco: [35, 100],
  egypt: [30, 90],
  tunisia: [35, 95],
  "south-africa": [45, 120],
...
/** Region / subregion fallbacks (USD mid-range daily) */
export const REGION_MID_USD = {
...
/** Continent fallbacks */
export const CONTINENT_MID_USD = {
  Europe: [80, 180],
  Asia: [40, 120],
  Africa: [35, 100],
...
/** City slug overrides (tourist premium / bargain hubs) — mid USD [low, high] */
export const CITY_MID_USD = {
  dubai: [130, 300],
  "abu-dhabi": [120, 280],
  tokyo: [110, 240],
  kyoto: [100, 220],
...
/**
 * @param {{ slug?: string, continent?: string, region?: string }} meta
...
/**
 * @param {[number, number]} mid
 * @returns {Tiers}
 */
export function tiersFromMid(mid, currency = "USD") {
  const [lo, hi] = mid;
  ...
  return {
    ...
    luxury,
    currency,
    note: `Typical ground costs for a mid-range traveller (~$${Math.round(midPoint)}/day). Hostel/street-food stays land near the budget tier; boutique hotels and fine dining push toward luxury.`,
  };
}

export function formatRange([lo, hi], currency = "USD") {
  if (currency === "USD") return `$${lo}–${hi}`;
  return `${lo}–${hi}`;
}

export function formatTierRange([lo, hi]) {
  return `${lo}–${hi}`;
}

/**
 * Country display string for budgetPerDay / compare.
 */
export function budgetPerDayForCountry(meta = {}) {
  return formatRange(midRangeForCountry(meta), "USD");
}

/**
 * Full tripCost for a city (or country capital proxy).
 * @param {{ slug?: string, name?: string, countrySlug?: string, continent?: string, region?: string, countryName?: string }} meta
 */
export function tripCostForPlace(meta = {}) {
  let mid;
  if (meta.slug && CITY_MID_USD[meta.slug]) mid = clampBand(...CITY_MID_USD[meta.slug]);
  else mid = midRangeForCountry({ slug: meta.countrySlug, continent: meta.continent, region: meta.region });

  const tiers = tiersFromMid(mid, "USD");
  const place = meta.name || "this destination";
  return {
    currency: "USD",
    budget: formatTierRange(tiers.budget),
    mid: formatTierRange(tiers.mid),
    luxury: formatTierRange(tiers.luxury) + "+",
...
/**
 * Split a mid-range daily total into calculator line items (USD).
 * Accommodation is per night (shared room assumption for 2 is handled by UI).
 */
export function calculatorDefaultsFromMid(mid) {
  const [lo, hi] = mid;
  const daily = Math.round((lo + hi) / 2);
  // Mid-range split (approx): lodging 50%, food 28%, transport 10%, activities 12%
  return {
    ...
    // Flights vary wildly — keep a neutral default; label as estimate
    flights: 600,
  };
}
...
export function calculatorDefaultsForCity(meta = {}) {
  const mid =
    meta.slug && CITY_MID_USD[meta.slug]
      ...
      : midRangeForCountry({ slug: meta.countrySlug, continent: meta.continent, region: meta.region });
  return calculatorDefaultsFromMid(mid);
}

/** Parse "$80–180" or "80–180" → [80, 180] */
export function parseBudgetPerDay(str) {
  if (!str) return null;
